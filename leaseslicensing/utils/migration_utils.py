...
from leaseslicensing.components.proposals.models import Proposal, ProposalType, ProposalOtherDetails, ProposalPark
from leaseslicensing.components.approvals.models import Approval
from leaseslicensing.components.bookings.models import ApplicationFee, ParkBooking, Booking
...
def run_deploy(tclass_csv, eclass_csv):
    """
    tclass_csv: 'leaseslicensing/utils/csv/Commercial-Licences-Migration-20191119.csv'
    eclass_csv: 'leaseslicensing/utils/csv/E-Class-20191119.csv'
    """
    clear_applications()
...
def clear_applications():
    def reset_sql_idx():
        ...
        cursor.execute('''ALTER SEQUENCE leaseslicensing_applicationfee_id_seq RESTART WITH 1''')
        cursor.execute('''ALTER SEQUENCE leaseslicensing_parkbooking_id_seq RESTART WITH 1''')
        cursor.execute('''ALTER SEQUENCE leaseslicensing_booking_id_seq RESTART WITH 1''')
        cursor.execute('''ALTER SEQUENCE leaseslicensing_approval_id_seq RESTART WITH 1''')
        cursor.execute('''ALTER SEQUENCE leaseslicensing_proposal_id_seq RESTART WITH 1''')
...
class OrganisationReader():
    ...
    Usage:
        from leaseslicensing.utils.migration_utils import OrganisationReader
...
#                data.update({'start_date': datetime.date.today()})
        ...
        try:
            ...
            To test:
                from leaseslicensing.components.proposals.models import create_organisation_data
                create_migration_data('leaseslicensing/utils/csv/orgs.csv')
            '''
