...
def search(dictionary, search_list):
    """
    To run:
            from leaseslicensing.utils import search
...
def compare_proposal(current_proposal, prev_proposal_id):
    ...
    return compare_data(
        current_proposal.data[0], prev_proposal.data[0], current_proposal.schema
    )
...
def compare_data(dict1, dict2, schema):
    ...
    To run:
            from leaseslicensing.utils import compare
...
def create_helppage_object(
    application_type="T Class", help_type=HelpPage.HELP_TEXT_EXTERNAL
):
    """
...
def search_keys(dictionary, search_list=["help_text", "label"]):
    ...
    To run:
            from leaseslicensing.utils import search_keys
...
def test_search_multiple_keys():
    ...
    return search_multiple_keys(
        p.schema, primary_search="isRequired", search_list=["label", "name"]
    )
