...
class Remittance(Flowable):
    ...
    def __logo_line(self):
        ...
        canvas.setFont(DEFAULT_FONTNAME, MEDIUM_FONTSIZE)
        canvas.drawString(
            current_x * 27, current_y, "PLEASE DETACH AND RETURN WITH YOUR PAYMENT"
        )
    ...
    def __payment_line(self):
        ...
        cheque_y -= 15
        canvas.drawString(
            cheque_x, cheque_y, "Mail to: Department of Parks and Wildlife"
        )
        cheque_y -= 15
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    booking = doc.booking
    licence_number = (
        booking.proposal.approval.lodgement_number
        if booking.proposal.approval
        else None
    )
    ...
    canvas.drawString(
        ...
        current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER),
        to_local_tz(booking.created).strftime(DATE_FORMAT) + " (AWST)",
    )
    canvas.drawString(
...
def _create_confirmation(confirmation_buffer, booking):
    ...
    doc = BaseDocTemplate(
        confirmation_buffer, pageTemplates=[every_page_template], pagesize=A4
    )
...
def create_monthly_confirmation_pdf_bytes(filename, booking):
    confirmation_buffer = BytesIO()
    _create_confirmation(confirmation_buffer, booking)
