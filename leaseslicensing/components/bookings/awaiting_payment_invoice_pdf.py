...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    if hasattr(proposal.invoice, 'payment_status') and proposal.invoice.payment_status in ['paid', 'over_paid']:
        canvas.drawString(current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 2, proposal.invoice.owner.get_full_name())
        canvas.drawString(current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 3, proposal.invoice.owner.email)
    current_x += 435
    ...
    if proposal.filming_fee_invoice_reference:
        canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 4, 'Invoice')
