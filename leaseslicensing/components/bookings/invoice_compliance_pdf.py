    ...
    def __payment_line(self):
        ...
        if self.invoice.payment_method in [
            ...
            canvas.rect(
                ...
                1.65 * inch,
                (bpay_logo_size[1] / 12) - 5,
            )
            # Bpay info
            canvas.setFont(BOLD_FONTNAME, MEDIUM_FONTSIZE)
            info_y = ((current_y - (bpay_logo_size[1] / 12 * 1.7)) + 3) + (0.35 * inch)
            canvas.drawString(
                biller_x + 5, info_y, "Biller Code: {}".format(self.invoice.biller_code)
            )
            canvas.drawString(
                biller_x + 5, info_y - 20, "Ref: {}".format(self.invoice.reference)
            )
            # Bpay Info string
            canvas.setFont(BOLD_FONTNAME, SMALL_FONTSIZE)
            canvas.drawString(
                box_pos, info_y - 0.55 * inch, "Telephone & Internet Banking - BPAY"
            )
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    canvas.drawCentredString(
        PAGE_WIDTH / 2, current_y - LARGE_FONTSIZE, "ABN: 38 052 249 024"
    )
    ...
    canvas.drawString(
        current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER), "Date"
    )
    canvas.drawString(
    ...
    canvas.drawString(
        current_x + invoice_details_offset,
        current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 3,
        compliance.proposal.lodgement_number,
    )
    canvas.drawRightString(
        current_x + 20,
    ...
    if cfi and invoice.payment_method in [
        ...
        canvas.drawRightString(
            current_x + 20,
            current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 10,
            "Payment Due Date",
        )
        canvas.drawString(
            current_x + invoice_details_offset,
...
def _create_invoice(invoice_buffer, invoice, compliance):
    ...
    desc = "${} per participant licence charge - {} participants ".format(
        compliance.proposal.application_type.events_park_fee,
        compliance.num_participants,
    )
    qty = 1
    data.append(
        [
