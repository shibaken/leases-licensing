...
class Remittance(Flowable):
    def __init__(self, current_x, current_y, compliance, invoice):
        Flowable.__init__(self)
    ...
    def __footer_line(self):
        ...
        canvas.drawString((PAGE_WIDTH/4) * 2, current_y, currency(self.invoice.amount - calculate_excl_gst(self.invoice.amount) if not _is_gst_exempt(self.compliance, self.invoice) else 0.0))
...
def _create_header(canvas, doc, draw_page_number=True):
    ...
    if compliance.proposal.org_applicant:
        canvas.drawString(current_x, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER),    compliance.proposal.applicant)
    ...
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 3, compliance.proposal.lodgement_number)
    canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 4, 'Compliance Number')
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 4, compliance.lodgement_number)
    canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 5, 'Invoice Number')
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 5, invoice.reference)
    canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 6, 'Total (AUD)')
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 6, currency(invoice.amount))
    canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 7, 'GST included (AUD)')
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 7, currency(invoice.amount - calculate_excl_gst(invoice.amount) if not _is_gst_exempt(compliance, invoice) else 0.0))
    canvas.drawRightString(current_x + 20, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 8, 'Paid (AUD)')
    canvas.drawString(current_x + invoice_details_offset, current_y - (SMALL_FONTSIZE + HEADER_SMALL_BUFFER) * 8, currency(invoice.payment_amount))
...
def _is_gst_exempt(compliance, invoice):
    # TODO need to fix, since individual parks can be exempt, Below calculation assumes NO PARK IS exempt
    return compliance.proposal.application_type.is_gst_exempt if compliance.fee_invoice_reference == invoice.reference else False

def _create_invoice(invoice_buffer, invoice, compliance):
...
#        val += 1
    ...
    remittance = Remittance(HEADER_MARGIN,HEADER_MARGIN - 10, compliance, invoice)
...
def create_invoice_compliance_pdf_bytes(filename, invoice, compliance):
    invoice_buffer = BytesIO()
    _create_invoice(invoice_buffer, invoice, compliance)
