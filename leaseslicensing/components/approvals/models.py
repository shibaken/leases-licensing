...
#            return None
    ...
    def generate_doc(self, user, preview=False):
        from leaseslicensing.components.approvals.pdf import create_approval_doc, create_approval_pdf_bytes
...
#        copied_to_permit = self.copiedToPermit_fields(self.current_proposal) #Get data related to isCopiedToPermit tag
    ...
    def approval_surrender(self,request,details):
        with transaction.atomic():
            try:
                if not request.user.leaseslicensing_organisations.filter(organisation_id = self.applicant_id):
