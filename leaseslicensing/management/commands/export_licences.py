...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open('/tmp/export_licences.csv', 'w') as f: 
            ...
            f.write('Status; Lodgement Number; Application Number; Start Date; Expiry Date; Applicant; ABN; Trading Name; Organsiation Email; Contacts; Application Type; Migrated\n')
            for a in Approval.objects.all():
                status = a.get_status_display()
                lodgement_number = a.lodgement_number if hasattr(a, 'lodgement_number') else 'None'
                ...
                org_applicant = a.org_applicant.organisation.trading_name if a.org_applicant.organisation.trading_name else 'None'
                email = a.current_proposal.org_applicant.email if a.current_proposal.org_applicant.email else 'None'
                contacts = ', '.join(a.org_applicant.contacts.all().values_list('email', flat=True))
                application_type = a.current_proposal.application_type.name
                migrated = 'True' if a.current_proposal.migrated else 'False'

                out_str = '; '.join([status, lodgement_number, application_number, start_date, expiry_date, applicant, abn, org_applicant, email, contacts, application_type, migrated])
